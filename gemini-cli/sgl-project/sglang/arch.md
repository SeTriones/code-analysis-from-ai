...
Based on the code analysis, here is the detailed execution of the **Verify Process** in SGLangs speculative decoding (specifically the EAGLE implementation).
...
1.  **Input Swap:** The batchs `input_ids` are replaced with `draft_token` (the list of all candidate tokens).
...
4.  **Bonus Token:** After the accepted path ends, the kernel samples one additional token from the target models distribution (the "bonus" token).
...
This is a critical performance step in SGLangs implementation.
1.  **State Update:** Accepted tokens are appended to the requests `output_ids`.
...
### `EagleVerifyInput`
Prepares the batch for the target models verification pass.
